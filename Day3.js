...
const target_puzzle = puzzle; //test_puzzle;
...
// // const isSpecial = char => SPECIAL_CHARS.includes(char);

// const isValid = (num) => {
//     const {x, y, len} = num;

//     if (x-1 >= 0 && isSymbol(target_puzzle[y][x-1])) // check left
//         return true;
//     if (x+len < target_puzzle[y].length && isSymbol(target_puzzle[y][x+len])) // check right
//         return true;
//     if (y-1 >= 0) {// check top
//         for (let i=-1; i<len+1; i++) {
//             if (x+i >= 0 && x+i < target_puzzle[y-1].length && isSymbol(target_puzzle[y-1][x+i]))
//                 return true;
//         }
//     } 
//     if (y+1 < target_puzzle.length) {// check bottom
//         for (let i=-1; i<len+1; i++) {
//             if (x+i >= 0 && x+i < target_puzzle[y+1].length && isSymbol(target_puzzle[y+1][x+i]))
//                 return true;
//         }
...
// const getRowValues = () => target_puzzle.map((row, index) => {
//     const num_objs = row
//         .replaceAll('&', '.')
//         .replaceAll('+', '.')
//         .replaceAll('*', '.')
...
//             len: _.length,
//             val: parseInt(_)
//         })) // format with original index, str length and int value
//         .filter(_ => _ && !isNaN(_.val)) // filter out non-numbers
//         .map(num => ({
//             ...num,
//             valid: isValid(num)
//         }))
...
//     const nums = num_objs.map(_ => _.val);
//     const sum = nums.reduce((a, b) => a + b, 0);

//     console.debug(`row ${index+1}:`, sum, nums)
//     return sum;
// })

// const total = getRowValues().reduce((a, b) => a + b, 0);
...
console.log('sum:', part1(target_puzzle))