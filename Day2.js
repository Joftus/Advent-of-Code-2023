...
// const valid_games = games.map((game, index) => {
//     game = game.split(': ')[1];
//     let rounds = game.split(';'); 
//     let valid = true;
//     rounds.forEach(round => round.split(', ').map(_ => {
//         const [amount, color] = _.trim().split(' ');
//         if (parseInt(amount) > available[color])
//             valid = false;
//     }))

//     if (!valid) {
//         console.debug('failed game:', index+1)
//         return 0;
//     }
...
// const part_1 = valid_games.reduce((a, b) => a + b, 0);
// console.debug('part 1:', part_1, valid_games)
// #endregion Part 1



// #region Part 2
const game_trackers = games.map(game => {
    game = game.split(': ')[1];
    let rounds = game.split(';');
    const tracker = { blue: -1, red: -1, green: -1 }
    rounds.forEach(round => round.split(', ').map(_ => {
        const [amount, color] = _.trim().split(' ');
        if (parseInt(amount) > tracker[color])
            tracker[color] = parseInt(amount);
    }))

    return tracker;
})

const power = game => game.blue * game.red * game.green

console.debug('game_trackers:', game_trackers);
const game_powers = game_trackers.map(game => power(game))
const part_2 = game_powers.reduce((a, b) => a + b, 0);
console.debug('case 2:', part_2, game_powers)
// #endregion Part 2